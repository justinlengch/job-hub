  ...
  const handleGmailSetup = async () => {
    ...
    try {
      ...
      if (!provider_token || !provider_refresh_token) {
        setSetupStatus("No Gmail tokens found. Please sign in again with Gmail permissions.");
        return;
      }
      ...
      <CardContent className="space-y-4">
        {isEnabled ? (
          <div className="flex items-center gap-2 text-green-600">
            ...
            <span className="text-sm font-medium">Gmail automation is enabled</span>
          </div>
        ) : (
          <>
        ...
        {setupStatus && (
          <div className={`flex items-center gap-2 p-3 rounded-md ${
            setupStatus.toLowerCase().includes("success") || setupStatus.toLowerCase().includes("initiated")

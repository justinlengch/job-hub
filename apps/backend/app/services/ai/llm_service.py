...
def _get_fallback_output(notes: str = "Processing error occurred") -> LLMEmailOutput:
...
async def _call_gemini_with_retry(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if (
                "BlockedPromptError" in str(type(e).__name__)
...
async def extract_job_info(email_input: LLMEmailInput) -> LLMEmailOutput:
    ...
    logger.info(
        f"LLM request summary: subject_preview={email_input.subject[:80]!r} body_text_len={(len(email_input.body_text) if email_input.body_text else 0)} body_html_len={(len(email_input.body_html) if email_input.body_html else 0)}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"LLM classification: intent={result.intent.value} company={result.company} role={result.role} status={result.status.value} subject_preview={email_input.subject[:80]!r}"
            )
            return result
        ...
        try:
            ...
            logger.info(
                f"LLM classification (fallback): intent={result.intent.value} company={result.company} role={result.role} status={result.status.value} subject_preview={email_input.subject[:80]!r}"
            )
            return result
